...
export class AngularShapeView extends NodeView<AngularShape> {
  ...
  private setInstanceInput(content: Content, ref: EmbeddedViewRef<any> | ComponentRef<any>): void {
    ...
    if (content instanceof TemplateRef) {
      ...
      // componentRef.instance.setInput(ngArguments);
      componentRef.instance.data = ngArguments;
  ...
  protected renderAngularContent(): void {
    ...
    if (container) {
      ...
      const { injector, content, componentFactoryResolver } = registerInfo.get(node.shape)!;
      ...
      if (content instanceof TemplateRef) {
        ...
        const componentFactory = componentFactoryResolver.resolveComponentFactory(content);
