...
export class PublishModelComponent implements OnInit {
  ...
  handlePublishOk(): void {
    if (!this.planNodeChildList?.length && !this.setOfCheckedId?.size) {
  ...
  publishRequestStatus(statusParams): void {
    this.listService.modelPublishStatus(statusParams).subscribe(res => {
      if (res.code === '200') {
        this.tipTitle = res.message != 'end' ? this.sanitizer.bypassSecurityTrustHtml(res.message) : "正在发布..."
        if (res.message == 'end') {
          clearInterval(this.timer);
          this.timer = null;
          this.isSpinning = false;
